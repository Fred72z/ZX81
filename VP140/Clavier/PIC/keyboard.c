...
#pragma bit LED_repet @ PORTC.7
#pragma bit LED_rom @ PORTC.6
...
uns8 test_RET()	// teste si la touche Retour est appuyée
{
	uns8 i,r;
	r=1;
	for (i=0;i<8;i++)
	switch(i)
		{
			case 6  : if (MAT[i]!=0x08) r=0; break;
			default : if (MAT[i]!=0x00) r=0;
		}
	return r;
}
...
uns8 test_SUIV()	// teste si la touche Suite est appuyée
{
	uns8 i,r;
	r=1;
	for (i=0;i<8;i++)
	switch(i)
		{
			case 7  : if (MAT[i]!=0x08) r=0; break;
			default : if (MAT[i]!=0x00) r=0;
		}
	return r;
}

uns8 test_ANNUL()	// teste si la touche Annulation est appuyée
{
	uns8 i,r;
	r=1;
	for (i=0;i<8;i++)
	switch(i)
		{
			case 1  : if (MAT[i]!=0x01) r=0; break;
			default : if (MAT[i]!=0x00) r=0;
		}
	return r;
}
...
void mod_SHIFT()	// Si la touche Shift est appuyée avec une des touches de ponctuation
					// alors le code est changé vers les caractères oranges
{
	uns8 i,r;

	if ((MAT[7] & 1)==0x01)	// Shift appuyée
		{
			if (MAT[6]==0x10) {MAT[6]=0x20;}			// 8 => (
			if (MAT[6]==0x80) {MAT[5]=0x20;MAT[6]=0;}	// 9 => )


			if (MAT[4]==0x02) {MAT[1]=0x10;MAT[4]=0;}	// ? => /
			if (MAT[5]==0x02) {MAT[3]=0x10;MAT[5]=0;}	// : => *
			if (MAT[6]==0x02) {MAT[5]=0x40;MAT[6]=0;}	// - => =
			if (MAT[7]==0x03) {MAT[6]=0x40;MAT[7]=1;}	// ; => +
			if (MAT[3]==0x08) {MAT[4]=0x40;MAT[3]=0;}	// . => >
			if (MAT[1]==0x08) {MAT[0]=0x10;MAT[1]=0;}	// , => <
...
void maj_LED()
{
 if (repet==1) LED_repet=1; else LED_repet=0;	// commande la led repetition du clavier
 if (ZXrom==1) LED_rom=1; else LED_rom=0;			// commande la led rom du clavier
